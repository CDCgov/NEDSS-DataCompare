...
from validator.validator_setup import TableUIDValidator
from validator.validator_runner import TableRecordValidator
...
def main():
    ...
    try:
        # Initialize and load UID validator
        logger.info("Initializing TableUIDValidator...")
        ...
        if args.target_tables:
            ...
            logger.info("No target tables specified. Using all tables from CSV.")
            table_list = validator.get_all_tables_with_uid_columns()
        
        if table_list:
            validation_result = validator.validate_input_table_list(table_list) if args.target_tables else {'found_tables': [{'table_name': t, 'column_count': len(validator.get_uid_columns_for_table(t))} for t in table_list], 'total_requested': len(table_list), 'found_count': len(table_list), 'missing_count': 0, 'missing_tables': []}
            ...
            for table_info in validation_result['found_tables']:
                logger.info(f"    {table_info['table_name']}: {table_info['column_count']} UID columns")
            ...
            if validation_result['found_count'] > 0:
                ...
                try:
                    ...
                    record_validator = TableRecordValidator()
                    ...
                    record_validator.run_validation(found_table_names)
